...
def load_model_energy():
    ...
    return {
        ...
        "E_Gold_100_loss": df["E_gold_100_loss"].to_numpy(dtype=float),
        "E_Gold_100_wall_loss": df["E_gold_100_wall_loss"].to_numpy(dtype=float),
...
def plot_front_surface(output_dir, wall_name, simulation_data, model_data, front_times_ns):
    ...
    # Calculate optimal figure size so the time slices stack vertically.
    fig_width = 6.6
    fig_height = 2.2 * n_plots + 0.9
    ...
    for index, time_ns in enumerate(front_times_ns):
        ...
        sim_line, = ax.plot(
            simulation_data["r_cm"],
            simulation_data["profiles"][simulation_key],
            color="blue",
            linestyle="--",
            linewidth=2.6,
        ...
        henyey_line, = ax.plot(
            model_data["regular"][time_ns]["r_cm"],
            z_reg,
            color="firebrick",
            linestyle="--",
            linewidth=2.4,
            label=f"Model Henyey t={time_ns:.1f} ns",
        )
        flattop_line, = ax.plot(
            model_data["flattop"][time_ns]["r_cm"],
            z_flat,
            color="red",
            linestyle="-",
            linewidth=2.6,
            label=f"Model flattop t={time_ns:.1f} ns",
        )

        legend_handles.extend([sim_line, henyey_line, flattop_line])
        legend_labels.extend([
            f"Simulation t={time_ns:.1f} ns",
            f"Model Henyey t={time_ns:.1f} ns",
            f"Model flattop t={time_ns:.1f} ns",
        ])
...
def plot_energy(output_dir, wall_name, simulation_data, model_data):
    ...
    elif wall_name == "Gold_100":
        # Regular (Henyey)
        plt.plot(model_data["time_ns"], model_data["E_Gold_100_loss"], color="firebrick", linestyle="--", linewidth=2.4, label="Model E Gold loss (Henyey)")
        plt.plot(model_data["time_ns"], model_data["E_Gold_100_wall_loss"], color="brown", linestyle="--", linewidth=2.4, label="Model E Gold wall loss (Henyey)")
        # Flattop
        plt.plot(model_data["flattop_time_ns"], model_data["E_Gold_100_loss_flattop"], color="red", linestyle="-", linewidth=2.6, label="Model E Gold loss (flattop)")
        plt.plot(model_data["flattop_time_ns"], model_data["E_Gold_100_wall_loss_flattop"], color="crimson", linestyle="-", linewidth=2.6, label="Model E Gold wall loss (flattop)")
    else:
        # Regular (Henyey)
...
def plot_all_coatings_front_surface_at_time(configs, time_ns=COMBINED_TIME_NS):
    ...
    for config in configs:
        ...
        try:
            ...
            simulation_key = resolve_profile_time_key(simulation_data["profiles"], time_ns)
            model_data = load_model_front_surface(wall_name, [time_ns])
            ...
            plt.plot(
                model_data["regular"][time_ns]["r_cm"],
                model_data["regular"][time_ns]["z_F_radial_cm"],
                color=color,
                linestyle="-",
                linewidth=2.2,
            ...
            print(f"[{wall_name}] skipped in combined {time_ns:.1f} ns plot: {exc}")
    ...
    output_path = Path(__file__).resolve().parent / f"all_coatings_front_surface_t{time_ns:.0f}ns.png"
